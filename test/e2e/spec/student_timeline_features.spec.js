...
describe("Student 1",function(){
	...
	describe("Adding/Deleting confused in timeline",function(){  
		...
		it("should seek video to 65% & add confused item",function(){
			video.seek(65)
			sleep(5000)
	...
	describe("Markers in timeline && Student annotations display",function(){  
		...
		it("should add empty maker by shortcut ", function() {
			...
			video.seek(54)
			video.current_time.then(function(result){expect(result).toEqual("0:02:33")} )
			video.play()
			sleep(4000)
			...
			video.pause()
			video.seek(58)
			video.current_time.then(function(result){expect(result).toEqual("0:02:44")} )
			video.play()
			sleep(4000)
			expect(student_lec.annotation.isPresent()).toEqual(false)
			video.current_time.then(function(result){expect(result).toEqual("0:02:48")} )
			video.pause()
			video.seek(63)
			video.current_time.then(function(result){expect(result).toEqual("0:02:58")} )
			video.play()
			sleep(4000)
			expect(student_lec.annotation.isPresent()).toEqual(false)
			video.current_time.then(function(result){expect(result).toEqual("0:03:02")} )

			video.pause()
			video.seek(68)
			video.current_time.then(function(result){expect(result).toEqual("0:03:13")} )
			video.play()
			sleep(4000)
			expect(student_lec.annotation.isPresent()).toEqual(true)
			video.current_time.then(function(result){expect(result).toEqual("0:03:16")} )
			video.pause()
	    })
		it("should close timeline",function(){
			student_lec.close_timeline()
