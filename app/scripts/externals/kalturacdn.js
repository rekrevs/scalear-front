...
window['SCRIPT_LOADER_URL'] = 'http://cdnapi.kaltura.com/html5/html5lib/v2.56/load.php';
(function($) {
    "use strict";
    if (window.kWidget) {
        return;
    ...
    var kWidget = {
            ...
            if (this.widgetOriginalSettings[widgetId]) {
                player.kBind('kdpEmpty', function() {
                    player.sendNotification('changeMedia', {
                        'entryId': _this.widgetOriginalSettings[widgetId].entry_id
                    });
                });
            }
        ...
        embed: function(targetId, settings) {
            ...
            if (document.URL.indexOf('forceKalturaNativeComponentPlayer') !== -1 || document.URL.indexOf('forceKalturaNative') !== -1) {
                settings.flashvars["nativeCallout"] = {
                    plugin: true
                }
            }
            if (!settings.wid) {
...
kWidget.addUserAgentRule = function(uiconfId, rule, action) {
    ...
    if (kWidget.userAgentPlayerRules[uiconfId]) {
        ...
        kWidget.userAgentPlayerRules[uiconfId] = {
            'rules': {},
            'actions': {}
        };
    }
    var ruleIndex = parseInt(ruleInx) + 1;
    kWidget.userAgentPlayerRules[uiconfId]['rules'][ruleIndex] = {
        'regMatch': rule
    };
    kWidget.userAgentPlayerRules[uiconfId]['actions'][ruleIndex] = {
        'mode': action,
        'val': 1
    };
}
...
mw.setConfig('Mw.XmlProxyUrl', 'http://cdnapi.kaltura.com/html5/html5lib/v2.56/simplePhpXMLProxy.php');
...
mw.setConfig('Kaltura.kWidgetPsUrl', 'http://cdnapi.kaltura.com/html5/html5lib/v2.56/../kwidget-ps/ps');
...
kWidget.addUserAgentRule('12905712', '/.*/', 'leadWithHTML5');
kWidget.appendScriptUrls([], function() {
    window.kWidget.inLoaderUiConfJsCallback();
})
kWidget.uiConfScriptLoadList['12905712'] = 1;
kWidget.setup();
