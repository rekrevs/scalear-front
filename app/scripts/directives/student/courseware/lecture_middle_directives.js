...
angular
  ...
  .directive("studentAnswerForm", [
    ...
    "$stateParams",
    "$log",
    function(Lecture, $stateParams, $log) {
      return {
        scope: {
          quiz: "=",
          studentAnswers: "=",
        ...
        restrict: "E",
        template:
          "<ng-form name='qform'><div style='text-align:left;margin:10px;'>" +
          ...
          scope.updateValues = function() {
            ...
            if (
              (scope.quiz.quiz_type == "html" ||
                scope.quiz.quiz_type == "html_survey") &&
              scope.quiz.question_type == "OCQ"
            ) {
        ...
        link: function(scope) {
          scope.$watch("explanation[answer.id]", function(newval) {
            if (scope.explanation && scope.explanation[scope.answer.id]) {
              scope.mypop = {
                title:
                  '<b ng-class="{\'green_notification\':explanation[answer.id][0]==true, \'red_notification\':explanation[answer.id][0]==false}">{{explanation[answer.id][0]==true?("lectures.correct"|translate) : ("lectures.incorrect"| translate)}}</b>',
      ...
      link: function(scope) {
        scope.$watch("explanation[quiz.id]", function(newval) {
          if (scope.explanation && scope.explanation[scope.quiz.id]) {
            scope.mypop = {
              content:
                '<div ng-bind-html="explanation[quiz.id][answer]"></div>',
              html: true,
              trigger: "hover",
              container: "body"
            };
  ...
  .directive("studentAnswer", [
    "$rootScope",
    "$translate",
    "$log",
    "$timeout",
    function($rootScope, $translate, $log, $timeout) {
      return {
        replace: true,
        restrict: "E",
  ...
  .directive("studentDrag", [
    "$rootScope",
    "$translate",
    "$log",
    "ScalearUtils",
    function($rootScope, $translate, $log, ScalearUtils) {
      return {
        restrict: "E",
        ...
        link: function(scope, elem) {
          ...
          scope.formatDropped = function(event, ui) {
            ...
            else ui.draggable.css("background-color", "orange");
          };
          ...
          scope.setDropped = function(event, ui) {
            if (!scope.studentAnswers[scope.quiz.id][scope.data.id]) {
              ...
              scope.studentAnswers[scope.quiz.id][
                scope.data.id
              ] = ui.draggable.html();
          ...
          var destroyPopover = function(draggable) {
            if (scope.explanation_pop) draggable.popover("destroy");
          };

          scope.$watch("explanation[data.id]", function(newval) {
            if (scope.explanation && scope.explanation[scope.data.id]) {
              scope.selected_id = angular
                .element(elem.children()[1])
                .attr("id");
              var ontop = angular.element(".ontop");
              scope.explanation_pop = {
                title:
                  "<b ng-class='{green_notification: explanation[selected_id][0], red_notification: !explanation[selected_id][0]}'>{{explanation[selected_id][0]?('lectures.correct'|translate):('lectures.incorrect'|translate)}}</b>",
                content:
                  "<div ng-bind-html='explanation[selected_id][1]'></div>",
                html: true,
                trigger: $rootScope.is_mobile ? "click" : "hover",
                placement:
                  angular.element(elem.children()[1]).position().left >
                  ontop.width() / 2
                    ? "left"
                    : "right"
              };
              var bg_color = scope.explanation[scope.data.id][0]
                ? "darkseagreen"
                : "orangered";
              angular
                .element("#" + scope.data.id)
  ...
  .directive("studentTimeline", [
    "$timeout",
    "ContentNavigator",
    "TimelineFilter",
    function($timeout, ContentNavigator, TimelineFilter) {
      return {
        replace: true,
        restrict: "E",
        scope: {
        ...
        link: function(scope, element, attrs) {
          ...
          scope.$watch("open_timeline", function(status) {
            ...
            } else scope.delayed_timeline_open = status;
          });
  ...
  .directive("confusedTimeline", [
    "Lecture",
    "$filter",
    function(Lecture, $filter) {
      return {
        restrict: "A",
        scope: {
          data: "&",
  ...
  .directive("quizTimeline", [
    "OnlineQuiz",
    "$filter",
    "$rootScope",
    "$translate",
    "$log",
    function(OnlineQuiz, $filter, $rootScope, $translate, $log) {
      return {
        restrict: "A",
        scope: {
          data: "&",
        ...
        link: function(scope, element, attrs) {
          ...
          scope.formattedTime = $filter("format", "hh:mm:ss")(scope.item.time);
          scope.unsolved_msg = $translate.instant(
            "lectures.tooltip.unsolved_quiz"
          );
  ...
  .directive("notesTimeline", [
    ...
    "$state",
    "$filter",
    function($log, Lecture, $state, $filter) {
      return {
        restrict: "A",
        scope: {
          data: "&",
              ...
              Lecture.deleteNote(
                {
                  ...
                  lecture_id:
                    scope.item.data.lecture_id || $state.params.lecture_id,
        ...
        link: function(scope, element) {
          ...
          scope.moveCursorToEnd = function() {
            ...
            shortcut.add(
                "shift+enter",
                function() {
                 $('div.editable-input>p.medium-editor-p').html('<br>'+$('div.editable-input>p.medium-editor-p').html());
                 $('div.ng-scope editable>span.ng-binding>p.medium-editor-p').html('<br>'+$('div.note>span.ng-binding>p.medium-editor-p').html());
                },
